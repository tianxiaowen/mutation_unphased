...
triobinwidth = len(trios)//40
def ceildiv(a, b):
    return -(-a // b)

if triobin == 11:
  trioid = range(10*triobinwidth+1, int(len(trios)/4+1))

if triobin != 11:
  trioid = range((triobin-1)*triobinwidth+1, triobin*triobinwidth+1)

for q in range(1,len(trioid)+1):
  i = trioid[q-1]
